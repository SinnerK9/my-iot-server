...
## W2D6 笔记——Hub + Redis 联动：在线状态同步

### 一、Day 5 留下的问题

Day 5 写好了 `SetDeviceOnline`/`SetDeviceOffline` 等 Redis 工具函数，但它们是**孤立**的——没人调它们。今天要解决"谁在什么时机调"。

三个同步时机：
1. 用户**第一个 WS 连接建立** → Redis 标记"该用户在线"
2. 用户**最后一个 WS 连接断开** → Redis 标记"该用户离线"
3. **心跳**（每 54s）→ Redis 刷新 TTL

### 二、isFirst / isLast 判断——一用户多连接的核心

Day 4 发现同一个 userID 只能存一个 Client，多标签页会互相覆盖。修法：`Clients` 从 `map[uint64]*Client` 改为 `map[uint64]map[*Client]bool`（嵌套 map 模拟 set 套 set）。
...
h.Mu.Lock()
if h.Clients[client.UserID] == nil {
    h.Clients[client.UserID] = make(map[*Client]bool)  // 用户第一个连接时创建内层 set
}
isFirst := len(h.Clients[client.UserID]) == 0  // ← 在插入之前判断
h.Clients[client.UserID][client] = true
h.Mu.Unlock()

if isFirst {
    h.markUserOnline(client.UserID)  // 只有第一个连接才写 Redis
}
```
...
```go
h.Mu.Lock()
delete(set, client)            // 从内层 set 删掉这个连接
close(client.Send)
isLast := len(set) == 0        // ← 在删除之后判断
if isLast {
    delete(h.Clients, client.UserID)  // 删掉外层 key
}
h.Mu.Unlock()

if isLast {
    h.markUserOffline(client.UserID)  // 只有最后一个连接断开才删 Redis
}
```

**为什么 isFirst/isLast 的判断必须在 Lock 内部，但 Redis 调用在 Lock 外部**：`len(set)` 必须在锁持有下判断——因为在锁外面 set 可能被别的 goroutine 改掉。Redis 调用放锁外面——因为 Redis 网络 I/O 可能很慢（几 ms），在锁里调 Redis 会让所有注册/注销操作排队等 I/O。

### 三、心跳刷新 Redis TTL

WritePump 已经有 `time.NewTicker(pingPeriod)` 每 54s 发一次 WebSocket Ping。在同一 ticker 里调用 `c.Hub.RefreshUserTTL(c.UserID)`——不增加任何额外的定时器或网络请求，TTL 刷新和 Ping 同频。

### 四、三层存储架构（面试关键图）

```
MySQL（持久化）   —  用户注册、设备归属
Redis（临时）     —  用户在线状态、设备在线状态（120s TTL）
Hub 内存（瞬时）  —  WS 连接表（进程重启清空）
```

三者分工：MySQL 记"你是谁"，Redis 记"你在不在"，Hub 记"你现在连着我"。

### 五、本日踩的 bug

1. **编译错误**：`h.markUserOffline()` 漏了传 `client.UserID` → 函数签名要求参数，少传就编译不过。

2. **逻辑错误（阴险）**：`RefreshUserTTL` 的 key 写成了 `fmt.Sprintf("%d", userID)` 丢了 `"user:"` 前缀 → Redis 里实际 key 是 `user:1`，EXPIRE 去找 `1`——找不到，静默返回 0，不报错。120 秒后用户在线状态自动消失，看起来像"用户莫名其妙离线了"。**修法**：改成 `fmt.Sprintf("user:%d", userID)`。

### 六、新增文件结构决策表

今天写代码之前我发现自己经常不知道"这个功能应该放哪个文件"。总结一套规则：

| 我操作的数据 | 我做的事 | 放在哪 |
|---|---|---|
| HTTP request JSON / URL param | 解析请求 → 调 service → 返回 JSON | `handler/` |
| 多个 repository 组合 + 业务规则判断 | 校验 → 调 repo → 组合结果 | `service/` |
| 一张数据库表 | SQL 增删改查（纯粹的） | `repository/` |
| 数据库/Redis 连接本身 | 初始化、关闭、连接池 | `repository/db.go` 或 `redis.go` |
| struct 定义、tag | 数据结构 | `model/` |
| 不依赖业务的纯函数 | JWT、bcrypt | `pkg/` |
| WebSocket 连接、消息流转 | 连接管理 | `websocket/` |

**口诀**：摸到数据 → repository，摸到规则 → service，摸到请求 → handler，摸到路由 → main.go，不摸数据不摸请求 → 工具放 pkg/。
